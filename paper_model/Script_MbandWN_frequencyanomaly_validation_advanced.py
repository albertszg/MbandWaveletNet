...
if __name__ == '__main__':
    ...
    model = FMWN(kernelInit=3, m=m, k=k, level=level, kernelsConstraint='KI', mode=mode, device=device,
                 coefffilter=5.0, realconv=True, initHT=0.1, kernTrainable=True, threshold=True,t=30.0)
    model.cuda()
    ...
    print('Before'+'*' * 6 + 'threshold in first layer'+ '*' * 6)
    ...
    print('*' * 6 + 'threshold in first layer of band!' + '*' * 6)
    print(threshold_list[0].left)
    print(threshold_list[0].right)
    ...
    coefficient_visul(outTe[1], mode=mode, m=m,title='test')  # ,prefined_sort=[1,2,0,3]
