...
if __name__ == '__main__':
    ...
    model=FMWN(kernelInit=5,m=m,k=k,level=level,kernelsConstraint='KI',mode=mode,device=device,
               coefffilter=1.0,realconv=True,initHT=0.2,kernTrainable=True,threshold=False)
    model.cuda()

    opt = torch.optim.Adam(params=model.parameters() ,lr=0.00000000000000000003, betas=(0.9,0.999), eps=1e-07)
    ...
    for epoch in range(epochs):
        ...
        for i, sig in enumerate(train_iter):
            ...
            sig = sig.cuda()
            sigPred, coeff, loss2, _,_ = model(sig)
            loss1 = coeffLoss(coeff)
            r = recLoss(sig, sigPred)
            # loss = r
            # loss = 8.0*r +20.0*loss1 +10.0*loss2
            # loss = r + 1.0 * loss1 + 0.3 * loss2# 原来0.3
            loss = r + 0.3 * loss1 + 0.3 * loss2# 原来0.3
            # loss = r + 10.0*loss2
            # loss =  loss1
            # loss =  0.01 * loss2
            loss.backward()
