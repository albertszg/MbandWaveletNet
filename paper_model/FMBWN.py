    ...
    def forward(self, inputSig,noDHT=True,ht=None):#inputSig size: batch*channel*length*hight   hight = 1
        ...
        if self.mode=='DWT':
            ...
            for lev in range(self.level):
                ...
                all_coefficients.append(c_tmp[1:])
                c_tmp[1:] = self.thre[lev](c_tmp[1:], NodefineT=noDHT, HT=ht)
                coefficients.append(c_tmp[1:])
                visual_coefficients.append(c_tmp[1:])
                low_passed_signal = c_tmp[0]
                #back up
                #初始版本
                # c_tmp = self.MBAND[lev](low_passed_signal)
                ...
                # coefficients.append(c_tmp[1:] )#添加到系数集合，后面使用
        ...
        return re_signal,all_coefficients,vLossFilters,hard_threshold,visual_coefficients
