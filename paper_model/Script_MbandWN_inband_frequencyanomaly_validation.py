...
if __name__ == '__main__':
    ...
    model = FMWN(kernelInit=2, m=m, k=k, level=level, kernelsConstraint='KI', mode=mode, device=device,
                 coefffilter=5.0, realconv=True, initHT=0.01, kernTrainable=True, threshold=True,t=30.0)
    model.cuda()

    opt = torch.optim.Adam(params=model.parameters(), lr=0.003, betas=(0.9, 0.999), eps=1e-07)
    ...
    for epoch in range(epochs):
        ...
        for i, sig in enumerate(train_iter):
            ...
            sigPred, coeff, loss2, hard_threshold_max,_ = model(sig)
            loss1 = coeffLoss(coeff)#稀疏约束
            r = recLoss(sig, sigPred)
            # loss = r
            # loss = 8.0*r +20.0*loss1 +10.0*loss2
            # loss = r + 4.0 * loss1 + 0.3 * loss2
            loss = r + 0.3 * loss1 + 0.3 * loss2- 0.1*torch.log(hard_threshold_max+1) # 重构+稀疏+正交+阈值最大化
            # loss = r + 0.3*loss2
            # loss = r + 0.3*loss2 + 4.0*loss1# 重构+稀疏+正交
            # loss =  loss1
            # loss =  0.01 * loss2
            loss.backward()
            opt.step()
    ...
    for i, sig in enumerate(test_iter_A):
        ...
        loss_1 = recLoss(sig, out[0])
    print('signal A reconstruction error {}'.format(loss_1))
    ...
    for i, sig in enumerate(test_iter):
        ...
        loss_2 = recLoss(sig, outTe[0])
    print('signal B reconstruction error {}'.format(loss_2))
