...
if __name__ == '__main__':
    ...
    if generating_signal == 1:
        ...
        t = n / fs
        signal_1 = np.sin(10 * 2 * np.pi * t)
        signal_2 = np.sin(30 * 2 * np.pi * t)
        # signal_2 = np.where(signal_2 >= 0, 1, 0)
    ...
    model = FMWN(kernelInit=2, m=m, k=k, level=level, kernelsConstraint='PL', mode=mode, device=device,
                 coefffilter=5.0, realconv=True, initHT=0.001, kernTrainable=True, threshold=False,t=30.0)
    model.cuda()

    opt = torch.optim.Adam(params=model.parameters(), lr=3e-3, betas=(0.9, 0.999), eps=1e-07)
    ...
    fig.show()
    error_B = np.sum((signal[indPlot, 0, :, 0].squeeze()-out[0][indPlot, 0, :, 0].detach().cpu().numpy())**2)/1000
    print('A重构误差{}'.format(error_B))
    ...
    fig.show()
    error_B = np.sum((y-y1)**2)/1000
    print('B重构误差{}'.format(error_B))
